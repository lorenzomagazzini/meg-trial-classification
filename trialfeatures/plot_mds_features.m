function [ ] = plot_mds_features( features, trl_idx )
% Multi-dimensional scaling plots 

for i = 1:length(features)
    
    fields = fieldnames(features(i));
    exclude_features = {'chan_corr','wthn_chan_var','btwn_chan_var','wthn_chan_kurt','btwn_chan_kurt'}; %we only plot summary ones (otherwise takes ages to get distances)
    fields(ismember(fields,exclude_features)) = []; 
